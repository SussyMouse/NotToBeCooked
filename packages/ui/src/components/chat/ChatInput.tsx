...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const handleEditorKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (atOpen && totalFilteredCount > 0) {
      ...
      if (e.key === "ArrowUp") {
        e.preventDefault();
        setSelectedIndex((prev) => (prev - 1 + totalFilteredCount) % totalFilteredCount);
        return;
      }
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      ...
      <div className="relative">
        ...
        {atOpen && (
          <div className="absolute bottom-[calc(100%+6px)] left-0 right-0 bg-(--bg-raise,#1C2833) border border-(--line,#25313E) rounded-lg shadow-xl overflow-hidden z-30">
            <div className="px-3 py-1.5 border-b border-(--line-soft,#1B2530) font-mono text-[9.5px] uppercase tracking-wider text-(--tx-faint,#5C6976) flex items-center justify-between">
                  ...
                  <div
                    ...
                    className={`flex items-center gap-2 p-1.5 rounded cursor-pointer transition-colors ${
                      isSelected
                        ...
                        : "hover:bg-(--bg-hover,#213040) text-(--tx-dim,#8B98A7)"
                    }`}
                  >
        ...
        <div className="flex items-end gap-2 bg-(--bg-raise,#1C2833) border border-(--line,#25313E) focus-within:border-(--acc-deep,#1D5D8A) rounded-lg p-2 transition-colors relative">
          <div className="flex-1 min-w-0 relative">
            {isEmpty && (
              <div className="absolute top-0.5 left-0 text-[13.5px] text-(--tx-faint,#5C6976) pointer-events-none select-none">
            ...
            <div
              ...
              className="outline-none focus:outline-none focus-visible:outline-none focus:ring-0 focus-visible:ring-0 [outline:none] text-(--tx,#DCE3EA) text-[13.5px] min-h-6 max-h-36 overflow-y-auto leading-relaxed py-0.5 whitespace-pre-wrap wrap-break-word scrollbar-thin [scrollbar-color:var(--line,#25313E)_transparent]"
            />
          </div>
          ...
          <button
            ...
            title="Send Message"
            className="w-6.5 h-6.5 rounded flex items-center justify-center bg-(--acc-deep,#1D5D8A) hover:bg-(--acc-deep-h,#246C9E) text-(--on-acc,#EAF5FD) transition-colors flex-none cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-(--acc-deep,#1D5D8A) mb-0.5"
          >
