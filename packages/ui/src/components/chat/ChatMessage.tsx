        ...
        >
          {isPdf ? (
            <svg
              ...
              viewBox="0 0 16 16"
              fill="none"
              className="shrink-0 text-(--acc,#52A8EA) inline-block align-sub"
            >
              <path
  ...
  return (
    <div
      className={`flex flex-col gap-0.5 ${
        isMe ? "items-end" : "items-start w-full"
      } ${className}`}
    >
      ...
      <div
        className={`text-[13.5px] leading-relaxed ${
          isMe
            ? "bg-[rgba(var(--acc-rgb,82,168,234),0.12)] border border-[rgba(var(--acc-rgb,82,168,234),0.25)] text-(--acc-tx-soft,#A8D4F2) rounded-lg rounded-tr-xs px-3 py-2 max-w-[85%]"
            : "text-(--tx,#DCE3EA) w-full py-0.5 text-justify"
        } ${isOptimistic ? "opacity-70 animate-pulse" : ""}`}
      >
        {isMe ? (
          <div className="whitespace-pre-wrap">{renderMentionized(rawText)}</div>
        ) : (
          <div
        ...
        {/* Citations List */}
        {citations.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2 pt-1 border-t border-(--line-soft,#1B2530)/60">
            {citations.map((cite, cIdx) => (
              <button
                key={cIdx}
                type="button"
                onClick={() => onCiteClick?.(cite)}
                className="inline-flex items-center gap-1.5 bg-(--cite-bg,rgba(227,166,63,0.09)) border border-(--cite-line,rgba(227,166,63,0.38)) text-(--cite,#E3A63F) rounded px-2 py-0.5 text-[10.5px] font-mono hover:bg-[rgba(227,166,63,0.19)] transition-colors cursor-pointer outline-none focus:outline-none focus-visible:outline-none focus:ring-0 focus-visible:ring-0 [outline:none]"
              >
